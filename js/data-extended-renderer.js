...
function renderExtendedModule(moduleId) {
    ...
    const sectionsHtml = sections.map(sec => {
        ...
        if (sec.title.includes('🎥 Vídeos')) {
            ...
            body = `
                <p style="margin-bottom:14px;">Selección curada de vídeos en YouTube. Pulsa sobre una tarjeta para abrir los resultados actualizados de búsqueda:</p>
                ${renderVideoGrid(videos)}
            `;
            extraClass = 'module-section-full';
        ...
        // Las secciones con tablas/comandos largos también full-width
        if (body && (body.includes('<table') || body.length > 1800)) {
            extraClass = 'module-section-full';
        }

        return `
            <div class="module-section ${extraClass}">
                <h3>${sec.title}</h3>
                ${body}
            </div>
        `;
