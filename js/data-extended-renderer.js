...
// Sobrescribir renderModuleCard para que módulos extendidos muestren un badge visible
const originalRenderModuleCard = window.renderModuleCard;

window.renderModuleCard = function(m) {
    const progress = (typeof getModuleProgress === 'function') ? getModuleProgress(m.id) : 0;
    const stars = '⭐'.repeat(m.difficulty || 1);
    const extBadge = m.extended ? `
        <div class="module-extended-badge">
            ✨ AMPLIADO · Vídeos + Libros
        </div>
    ` : '';
    return `
        <div class="module-card ${m.extended ? 'module-card-extended' : ''}" onclick="openModule('${m.id}')">
            ${extBadge}
            ...
            ${m.extended ? `<div class="module-card-cta">📖 Abrir contenido completo →</div>` : ''}
        </div>
    `;
};
