...
function renderExercises(moduleId) {
    ...
    container.innerHTML = exercises.map((ex, idx) => {
        ...
        if (ex.type === 'multiple') {
            ...
            body = `
                <div class="exercise-options">
                    <button class="exercise-option" onclick="checkAnswer('${moduleId}', '${ex.id}', true)" ${completed ? 'disabled' : ''}>
                        ✅ Verdadero
                    </button>
                    <button class="exercise-option" onclick="checkAnswer('${moduleId}', '${ex.id}', false)" ${completed ? 'disabled' : ''}>
                        ❌ Falso
                    </button>
                </div>
            `;
    ...
    if (card) {
        ...
        buttons.forEach((btn, i) => {
            ...
            if (ex.type === 'multiple') {
                ...
                const val = btn.textContent.trim().startsWith('✅');
