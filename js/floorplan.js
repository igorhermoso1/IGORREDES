            ...
            <div class="fp-canvas-wrapper">
                <div class="fp-toolbar">
                    ...
                    <div class="fp-toolbar-actions">
                        <button class="fp-action-btn" onclick="fpTogglePanel('left')" title="Ocultar/mostrar panel izquierdo" id="fpToggleLeft">◀ Panel</button>
                        <button class="fp-action-btn" onclick="fpTogglePanel('right')" title="Ocultar/mostrar panel derecho" id="fpToggleRight">Panel ▶</button>
                        <button class="fp-action-btn" onclick="fpTogglePanel('both')" title="Maximizar zona de trabajo" id="fpToggleBoth">⛶ Maximizar</button>
...
// ============================================
// PLEGAR PANELES (más espacio para canvas)
// ============================================

let fpPanelState = { left: true, right: true };

function fpTogglePanel(which) {
    if (which === 'both') {
        // Si ambos visibles, ocultar ambos. Si alguno oculto, mostrar ambos.
        const anyHidden = !fpPanelState.left || !fpPanelState.right;
        fpPanelState.left = anyHidden;
        fpPanelState.right = anyHidden;
    } else if (which === 'left') {
        fpPanelState.left = !fpPanelState.left;
    } else if (which === 'right') {
        fpPanelState.right = !fpPanelState.right;
    }
    fpApplyPanelState();
    // Recentrar para aprovechar el espacio nuevo
    setTimeout(() => fpCenter(), 320);
}

function fpApplyPanelState() {
    const layout = document.querySelector('.fp-layout');
    if (!layout) return;
    layout.classList.remove('toolbox-hidden', 'panel-hidden', 'both-hidden');
    if (!fpPanelState.left && !fpPanelState.right) layout.classList.add('both-hidden');
    else if (!fpPanelState.left) layout.classList.add('toolbox-hidden');
    else if (!fpPanelState.right) layout.classList.add('panel-hidden');

    // Cambio de etiquetas de botones
    const lb = document.getElementById('fpToggleLeft');
    const rb = document.getElementById('fpToggleRight');
    const bb = document.getElementById('fpToggleBoth');
    if (lb) lb.textContent = fpPanelState.left ? '◀ Panel' : '▶ Panel';
    if (rb) rb.textContent = fpPanelState.right ? 'Panel ▶' : 'Panel ◀';
    if (bb) bb.textContent = (!fpPanelState.left && !fpPanelState.right) ? '⛶ Restaurar' : '⛶ Maximizar';
}
