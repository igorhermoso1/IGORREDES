...
function pingFrom(deviceId) {
    const device = simState.devices.find(d => d.id === deviceId);
    if (!device) return;
    const targets = simState.devices.filter(d =>
        d.id !== deviceId && SIM_DEVICE_TYPES[d.type].canPing && d.config.ip
    );
    if (targets.length === 0) {
        logToConsole(`❌ No hay otros dispositivos con IP a los que hacer ping.`, 'error');
        renderConfigPanel(deviceId);
        return;
    }

    const targetList = targets.map((t, i) => `${i + 1}. ${t.name} (${t.config.ip})`).join('\n');
    const choice = prompt(`📡 ¿A cuál hacer ping?\n\n${targetList}\n\nIntroduce el número:`);
    if (!choice) return;
    const target = targets[parseInt(choice) - 1];
    if (!target) return;
    sendPingBetween(device.id, target.id);
}
...
function sendPing() {
    ...
    const list = pingable.map((d, i) => `${i + 1}. ${d.name} (${d.config.ip})`).join('\n');
    const fromIdx = prompt(`📡 ORIGEN del ping:\n\n${list}\n\nNúmero:`);
    if (!fromIdx) return;
    const from = pingable[parseInt(fromIdx) - 1];
    if (!from) return;
    const targetList = pingable.filter(d => d.id !== from.id).map((d, i) => `${i + 1}. ${d.name} (${d.config.ip})`).join('\n');
    const otherDevices = pingable.filter(d => d.id !== from.id);
    const toIdx = prompt(`🎯 DESTINO del ping:\n\n${targetList}\n\nNúmero:`);
    if (!toIdx) return;
    const to = otherDevices[parseInt(toIdx) - 1];
    if (!to) return;
    sendPingBetween(from.id, to.id);
}
...
function saveTopology() {
    const name = prompt('Nombre para esta topología:', `Topología ${new Date().toLocaleString()}`);
    if (!name) return;
    const topology = {
        name,
        timestamp: Date.now(),
